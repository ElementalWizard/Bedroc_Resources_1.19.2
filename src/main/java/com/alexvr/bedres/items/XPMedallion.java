...
public class XPMedallion extends Item implements IDisplayFlux {
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        pTooltipComponents.add(new TextComponent("'K' by default changes modes"));
        pTooltipComponents.add(new TextComponent(String.format("Mode: %s",getMode(pStack) == 1?"Use":"Auto Absorb")).withStyle(ChatFormatting.DARK_PURPLE));
        pTooltipComponents.add(new TextComponent(String.format("Total Levels: %d",getXP(pStack))).withStyle(ChatFormatting.DARK_PURPLE));
    }
